...
def wls_filter_test(img, guide, alpha=1.2, Lambda=0.032):
    ...
    cv2.imwrite(path + 'grad_x.png', (gradWeightX * 255.0).astype('uint8'))
...
if __name__ == '__main__':
    img1 = 1.0 * cv2.imread(path+'output_img03.png') / 255.0
    dl_img = cv2.imread(path+'/input/in3.png')
    h, w, c = dl_img.shape
    new_width = 256

    k = float(w) / h
    dim = (256, 256)
    resized_img = cv2.resize(dl_img, dim, interpolation=cv2.INTER_AREA)
    img2 = 1.0 * resized_img / 255.0

    result = wls_filter(img1, img2, 1.2, 1) + img2 - wls_filter(img2, img2, 1.2, 1)
    result = np.clip(result, 0, 1)
    diff = img1 - result
    cv2.imwrite(path+'result0003.png', (result * 255.0).astype('uint8'))
    cv2.imwrite(path+'diff13.png', (diff).astype('uint8'))
